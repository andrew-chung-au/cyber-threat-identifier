...
Main landing page with sidebar info and navigation tabs.
"""
...
# Setup paths so src/ imports work when running via Makefile from project root
...
def render_sidebar() -> None:
    """Render sidebar with concise about + system info."""
    with st.sidebar:
        st.header("About")
        st.markdown("""
        ...
        **Feedback**
        ...
        st.caption("🤖 Gemini 3.5 Flash-Lite")
        st.caption("🔧 Retrieval: Vector + reranking")
...
def render_query_tab() -> None:
    ...
    try:
        ...
        st.warning(f"Query interface not available: {e}")
...
def render_dashboard_tab() -> None:
    ...
    try:
        ...
        st.warning(f"Dashboard not available: {e}")
...
def main() -> None:
    ...
    tab1, tab2, tab3 = st.tabs(["🏠 Home", "🔍 Query", "📊 Dashboard"])
    with tab1:
        render_home_tab()
    with tab2:
        render_query_tab()
