...
This runbook explains how to reproduce the current Cyber Threat Identifier ingestion, database, embedding, retrieval-benchmark, document-reranking, answer-generation, LLM-evaluation, and Streamlit UI baselines from a clean checkout.
...
Install Streamlit and plotting dependencies (required for the UI and dashboard):
...
This section reproduces the automated model-comparison experiments described in DEC‑020. It compares answer-generation outputs from `gemini-3.1-flash-lite` and `gemini-3.5-flash-lite` using LLM-as-judge evaluation.
...
This section explains how to run the Streamlit interface and monitoring dashboard.
...
- Completed the environment setup in the "Setup" section.
- Installed Streamlit and plotting dependencies:
  ```bash
  make install
  ```
- A running database with embeddings built (for the Query tab to work).
- Evaluation CSVs present if you want the dashboard charts to render:
  - `data/evaluation_reports/expert_llm_comparison_v1.csv`
  - `data/evaluation_reports/expert_llm_judged_35_as_judge.csv`
  - `data/evaluation_reports/judge_agreement_summary.csv`


If these files are missing, the dashboard will show error messages or placeholders for the affected charts.


### 2. Run the main Streamlit app


Run the main application (Home, Query, and Dashboard tabs):
...
- **Home tab**: Overview, data sources, and ATT&CK usage information.
- **Query tab**: Incident narrative input, retrieval with reranking, and structured answer generation.
- **Dashboard tab**: Monitoring dashboard with evaluation metrics and charts.


### 3. Run the monitoring dashboard standalone


Run the monitoring dashboard as a standalone page:
...
3. Retrieval method comparison (MRR & Hit@3).
4. Judge agreement rate.
5. User feedback distribution (placeholder until feedback persistence is implemented).


If evaluation CSVs are missing, the corresponding charts display error messages or info notices rather than failing the entire dashboard.


### 4. Run the full stack with Docker (optional)


To run PostgreSQL and Streamlit together using Docker Compose:
...
- A Streamlit container running the main app (`app/home.py`).


The Streamlit UI will be available at `http://localhost:${STREAMLIT_PORT:-8501}`.
...
- Feedback buttons in the Query tab are currently a UI prototype; submissions are not yet persisted to `data/feedback/feedback.csv`.
- The retrieval-comparison chart in the dashboard currently uses hard-coded metrics from DEC-018 and DEC-019; future versions can read dynamically from benchmark CSVs.
- The dashboard assumes evaluation reports exist; if files are missing, it displays informative error messages.
- The UI sidebar currently states a specific model (e.g., "Gemini 3.5 Flash-Lite"), but the actual model is environment-configured via `MODEL_ID`. This may be clarified in a future iteration.
...
### Streamlit UI or dashboard issues


If the Streamlit app fails to start:
...
If dashboard charts show errors:


- Confirm evaluation CSVs exist:
  ```bash
  ls -lh data/evaluation_reports/expert_llm_comparison_v1.csv
  ls -lh data/evaluation_reports/expert_llm_judged_35_as_judge.csv
  ls -lh data/evaluation_reports/judge_agreement_summary.csv
  ```
- If files are missing, run the corresponding evaluation scripts (see "Run pairwise LLM-as-judge evaluation").


If the dashboard shows "No feedback collected yet":


- This is expected; feedback persistence is not yet implemented.
