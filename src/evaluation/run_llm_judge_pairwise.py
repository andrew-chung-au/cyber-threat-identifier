...
- Built-in retry/backoff via llm_client
...
import pandas as pd
from src.llm_client import generate_text_answer
...
def run_judge_on_case(
    ...
    # Call judge LLM (retries handled inside generate_text_answer)
    response_text, usage = generate_text_answer(
        instructions=(
            "You are an expert cybersecurity analyst evaluating ATT&CK mapping answers. "
            "Be objective and detailed in your reasoning."
        ),
...
def main() -> None:
    ...
    parser.add_argument(
        "--judge-model",
        required=True,
        help="Model to use as judge (e.g., gemini-3.1-flash-lite)",
    )
    parser.add_argument(
        "--pause-between-calls",
        type=float,
        default=0.0,
        help="Extra pause between API calls (seconds)",
    )
    ...
    if args.output.exists():
        ...
        print("[INFO] Resuming from checkpoint, skipping completed cases...")
        output_rows = existing_df.to_dict("records")
    else:
        judged_eval_ids: set[str] = set()
        output_rows: list[dict[str, Any]] = []

    cases = input_df.groupby("eval_id")

    remaining_cases = [
        (eval_id, group) for eval_id, group in cases if eval_id not in judged_eval_ids
    ]
