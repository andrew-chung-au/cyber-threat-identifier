...
> - The answer-generation baseline is implemented and currently uses vector retrieval to produce structured, retrieval-grounded outputs; integration with reranked retrieval is planned.
> - Pairwise LLM-as-judge evaluation (3.1 vs 3.5) and cross-judge agreement analysis over 226 expert-derived cases are implemented (DEC-020); a final default model choice is deliberately deferred until targeted manual review of disagreement cases is completed.
> - An analyst-facing Streamlit UI and monitoring dashboard are implemented; feedback persistence and a frozen held-out external benchmark remain in progress.
...
- Streamlit analyst-facing UI and monitoring dashboard:
  - Main app (`app/home.py`) with Home, Query, and Dashboard tabs.
  - Query interface (`app/query.py`) for incident-to-ATT&CK mapping.
  - Monitoring dashboard (`app/dashboard.py`) with five evaluation charts.
...
- Implement analyst feedback persistence (thumbs up/down) and link it to logging/monitoring.
...
For answer-generation and LLM-as-judge evaluation commands (including `run_expert_answer_generation`, `run_llm_judge_pairwise`, and `analyze_judge_agreement`), see the detailed steps in [`docs/runbook.md`](docs/runbook.md).
...
- **Query**: Incident narrative input, retrieval with reranking, and structured answer generation.
...
│
├── app/                     # Streamlit UI
│   ├── home.py              # Main tabs (Home / Query / Dashboard)
...
| Document | Purpose |
|---|---|
| [`docs/runbook.md`](docs/runbook.md) | Setup, pipeline commands, verification, rebuilds, benchmarks, answer-generation, LLM-as-judge evaluation, UI, and troubleshooting |
...
| [`docs/decisions.md`](docs/decisions.md) | Stable architecture, corpus, retrieval, and evaluation decisions (e.g. DEC‑017, DEC‑018, DEC‑019, DEC‑020) |
...
- LLM-as-judge evaluation is not ground truth; both judges show a tendency to prefer `gemini-3.1-flash-lite` on the current dataset, and approximately 25% of cases involve judge disagreement.
- A final default answer-generation model has not yet been selected; a decision will be made only after targeted manual inspection of disagreement cases.
